...
def load_data(path_url:Path)->pd.DataFrame:
    try:
...
def change_columns_name(df:pd.DataFrame)->pd.DataFrame:
...
if __name__=="__main__":
    ...
    clean_data=root_path/"data"/"cleaned"
    clean_data.mkdir(exist_ok=True,parents=True)
    clean_data_filename="swiggy_cleaned.csv"
    dir_path=clean_data/clean_data_filename
    ...
    perform_data_cleaning(data=df,saved_data_path=dir_path)
